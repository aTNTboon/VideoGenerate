...
def simple_num_symbol_to_cn(text: str) -> str:
    def repl(ch):
        if ch in digit_map:
            return digit_map[ch]
        return ch
...
def self_parse(text, theme_id):
    ...
    for mood, content in scenes:
        # content_cn = simple_num_symbol_to_cn(content)

        # 构建正则：按结束符切分，并保留标点
        pattern = f"({'|'.join(map(re.escape, end_prefix))})"
        ...
        for part in parts:
            for char in part:
                if (char in end_prefix) and (len(buffer) >= 20):
                    buffer += char
                    new_scenes.append((mood, buffer.strip()))
                    buffer = ""
                elif ((char in continue_prefix) or (char in end_prefix)) and len(
                    buffer
                ) >= 30:
                    buffer += char
                    new_scenes.append((mood, buffer.strip()))
